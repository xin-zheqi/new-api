...
type ChannelSettings struct {
	...
	// FirstResponseTimeoutAutoBan only enables the first-response timeout path
...
type ChannelOtherSettings struct {
	...
	ClaudeCodeMimic          bool          `json:"claude_code_mimic,omitempty"`           // Claude 渠道是否伪装为 Claude Code CLI
	...
	DisableImageGeneration   bool          `json:"disable_image_generation,omitempty"`    // 是否禁止该渠道使用 OpenAI image_generation 工具或图片生成接口
	StripImageGenerationTool bool          `json:"strip_image_generation_tool,omitempty"` // 是否移除请求中显式传入的 image_generation 工具
	StripCodexImageTool      bool          `json:"strip_codex_image_tool,omitempty"`      // 兼容旧字段：是否移除 Codex 请求中显式传入的 image_generation 工具
	AwsKeyType               AwsKeyType    `json:"aws_key_type,omitempty"`
	// AutoTestEnabled controls whether this channel participates in scheduled
	// automatic tests. nil keeps the historical default of "enabled".
	AutoTestEnabled                       *bool                 `json:"auto_test_enabled,omitempty"`
