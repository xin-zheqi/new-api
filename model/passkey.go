...
func GetPasskeyByUserID(userID int) (*PasskeyCredential, error) {
	...
	if result.Error != nil {
		// 只有真正的数据库错误才记录日志
		common.SysLog(fmt.Sprintf("GetPasskeyByUserID: database error for user %d: %v", userID, result.Error))
		return nil, ErrFriendlyPasskeyNotFound
	}
...
func GetPasskeyByCredentialID(credentialID []byte) (*PasskeyCredential, error) {
	...
	if result.Error != nil {
		common.SysLog(fmt.Sprintf("GetPasskeyByCredentialID: database error for credential ID: %v", result.Error))
		return nil, ErrFriendlyPasskeyNotFound
	}
	if result.RowsAffected == 0 {
		return nil, ErrFriendlyPasskeyNotFound
