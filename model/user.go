...
func (user *User) InsertWithRewardPolicy(inviterId int, rewardPolicy RegistrationRewardPolicy) error {
	rewardPolicy = rewardPolicy.withDefaults()
	if err := DB.Transaction(func(tx *gorm.DB) error {
	...
	user.finishInsert(inviterId, rewardPolicy)
	return nil
}

func (user *User) finishInsert(inviterId int, rewardPolicy RegistrationRewardPolicy) {
	rewardPolicy = rewardPolicy.withDefaults()
	// 用户创建成功后，根据角色初始化边栏配置
...
func (user *User) FinishInsert(inviterId int) {
	user.finishInsert(inviterId, RegistrationRewardPolicy{})
}
