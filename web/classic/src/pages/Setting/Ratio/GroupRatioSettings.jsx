...
const OPTION_KEYS = [
  ...
  'group_status_setting.enabled_groups',
  'AutoGroups',
  'DefaultUseAutoGroup',
];
...
export default function GroupRatioSettings(props) {
  ...
  const handleGroupTableChange = useCallback(
    ({ GroupRatio, UserUsableGroups, GroupStatusEnabledGroups }) => {
      setInputs((prev) => ({
        ...prev,
        GroupRatio,
        UserUsableGroups,
        'group_status_setting.enabled_groups': GroupStatusEnabledGroups,
      }));
    },
    [],
  );
      ...
      <Form.Section text={t('分组管理')}>
        ...
        <GroupTable
          ...
          groupStatusEnabledGroups={
            inputs['group_status_setting.enabled_groups']
          }
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              label={t('分组状态展示')}
              placeholder={t(
                '为一个 JSON 文本，键为分组名称，值为是否展示状态',
              )}
              extraText={t(
                '分组状态展示配置，开启的分组会出现在分组状态页；未开启的分组不会展示',
              )}
              field={'group_status_setting.enabled_groups'}
              ...
              onChange={(value) =>
                setInputs((prev) => ({
                  ...prev,
                  'group_status_setting.enabled_groups': value,
                }))
              }
            />
          </Col>
